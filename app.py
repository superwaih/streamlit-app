...
def download_word_cloud(wc):
    if wc is not None:
        ...
        st.warning("Please generate the word cloud first.")
...
def main():
    ...
    if st.button("Generate Word Cloud"):
        if text:
            wc = generate_word_cloud(text, mask_image)
            if wc is not None:
                # Display the word cloud
                st.image(wc.to_image(), caption='Word Cloud')
                # Option to download the word cloud image
                st.button('Download Word Cloud Image', on_click=download_word_cloud, args=(wc,))
        else:
